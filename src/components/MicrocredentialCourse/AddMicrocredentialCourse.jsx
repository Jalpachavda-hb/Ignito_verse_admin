...
export default function AddMicrocredentialCourse() {
    ...
    async function loadDropdowns() {
      ...
      try {
        ...
        if (streamsRes.status === "fulfilled" && streamsRes.value?.success) {
          const list = streamsRes.value.streamDataList || [];
          setStreamOptions(list);
          if (list.length > 0) {
            setFormData((prev) => ({
              ...prev,
              microcredentialCourseStreamId: list[0].streamId || list[0].id || 0,
            }));
          }
        }

        if (levelsRes.status === "fulfilled" && levelsRes.value?.success) {
          const list = levelsRes.value.microCredentialCourseLevelList || [];
          setLevelOptions(list);
          if (list.length > 0) {
            setFormData((prev) => ({
              ...prev,
              microcredentialCourseLevelId:
                list[0].courseValue || list[0].microCredentialCourseLevelId || list[0].id || 0,
            }));
          }
        }
        ...
        if (langsRes.status === "fulfilled" && langsRes.value?.success) {
          ...
          if (list.length > 0) {
            setFormData((prev) => ({
              ...prev,
              languageId: list[0].languageId || list[0].id || 0,
            }));
          }
        }
      } catch (err) {
        console.warn("Error loading course dropdowns:", err);
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Upload Intro Image if selected (UploadSource: "IntroImage")
      if (introFile) {
      ...
      // 2. Upload Certificate Image if selected (UploadSource: "Certificate")
      if (certFile) {
      ...
      // 3. Build payload with adminId: 1
      const payload = {
        AdminId: 1,
        MicrocredentialCourseId: 0,
      ...
      if (res && res.success !== false) {
        const msg = res.message || "Microcredential course added successfully.";
        ...
        throw new Error(res?.message || res?.errorDescription || "Failed to add course.");
      }
    } catch (err) {
      console.error("Error creating course:", err);
      setErrorMessage(err.message || "Failed to create microcredential course.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="w-full pb-14">
      <PageMeta
        title="Add Microcredential Course | IgnitoVerse Admin"
        description="Create and configure a new microcredential course with syllabus, pricing, and certificate"
      />
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-5 dark:border-gray-800">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">
            Add New Microcredential Course
          </h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            Configure course curriculum, rich descriptions, certificate templates, and pricing.
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-12">
        ...
        <div className="space-y-6 xl:col-span-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 text-xs">
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 placeholder-gray-400 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:text-white"
                  required
                />
              </div>
          ...
          {/* Card 2: Descriptions & About with Rich Text Editor matching Screenshot 2 */}
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-xs dark:border-gray-800 dark:bg-gray-900">
            <h2 className="text-sm font-bold text-gray-900 dark:text-white mb-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              {/* Full Description with Rich Text Editor (CKEditor / Quill style) matching Screenshot 2 */}
              <div>
          ...
          {/* Card 3: What Will You Learn? & Material Include with Red Add Button matching Screenshot 2 */}
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-xs dark:border-gray-800 dark:bg-gray-900">
            <h2 className="text-sm font-bold text-gray-900 dark:text-white mb-4">
            ...
            <div className="mb-6 text-xs">
              ...
              {learnList.length > 0 ? (
                  ...
                  {learnList.map((item, idx) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="rounded-full text-red-500 hover:text-red-800"
                      >
                        ✕
                      </button>
                    </span>
                ...
                <p className="text-[11px] text-gray-400 italic">No learning points added yet.</p>
              )}
            </div>
            ...
            <div className="text-xs">
              ...
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  placeholder="e.g. 100+ Hours of On-Demand Video, Downloadable Practice Assets"
                  className="flex-1 rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-xs text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:text-white"
                />
                <button
                  type="button"
