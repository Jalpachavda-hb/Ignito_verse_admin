...
export default function EditTestimonialReview() {
  ...
  const [formData, setFormData] = useState({
    testimonialReviewMasterId: reviewId,
    reviewerName: passedItem?.reviewerName || passedItem?.ReviewerName || "",
    reviewerDesignation: passedItem?.reviewerDesignation || passedItem?.ReviewerDesignation || "",
    reviewInStar: Number(passedItem?.reviewInStar || passedItem?.ReviewInStar || 5),
    reviewDescription: passedItem?.reviewDescription || passedItem?.ReviewDescription || "",
    reviewerImagePath: passedItem?.reviewerImagePath || passedItem?.ReviewerImagePath || "",
    givenFileName: passedItem?.givenFileName || passedItem?.GivenFileName || "",
    originalFileName: passedItem?.originalFileName || passedItem?.OriginalFileName || "",
  });

  const [selectedFile, setSelectedFile] = useState(null);
  const [imagePreviewUri, setImagePreviewUri] = useState(
    formData.reviewerImagePath ? formatImageUrl(formData.reviewerImagePath) : ""
  );
  ...
  // Fetch freshest details from backend
  useEffect(() => {
    if (!reviewId || reviewId <= 0) {
      setErrorMessage("Invalid testimonial review ID.");
      setLoadingInitial(false);
      return;
    }

    setLoadingInitial(true);
    getTestimonialReviewById(reviewId, 0)
      .then((res) => {
        if (res?.success && res?.review) {
          ...
          setFormData({
            ...
            reviewInStar: Number(r.reviewInStar || 5),
            reviewDescription: r.reviewDescription || "",
            reviewerImagePath: r.reviewerImagePath || "",
            givenFileName: r.givenFileName || "",
            originalFileName: r.originalFileName || "",
          });
          if (r.reviewerImagePath) {
            setImagePreviewUri(formatImageUrl(r.reviewerImagePath));
          }
        } else if (res?.rawData) {
          const raw = res.rawData.data || res.rawData;
          setFormData((prev) => ({
            ...prev,
            reviewerName: raw.reviewerName || raw.ReviewerName || prev.reviewerName,
            reviewerDesignation: raw.reviewerDesignation || raw.ReviewerDesignation || prev.reviewerDesignation,
            reviewInStar: Number(raw.reviewInStar || raw.ReviewInStar || prev.reviewInStar),
            reviewDescription: raw.reviewDescription || raw.ReviewDescription || prev.reviewDescription,
            reviewerImagePath: raw.reviewerImagePath || raw.ReviewerImagePath || prev.reviewerImagePath,
            givenFileName: raw.givenFileName || raw.GivenFileName || prev.givenFileName,
            originalFileName: raw.originalFileName || raw.OriginalFileName || prev.originalFileName,
          }));
          const img = raw.reviewerImagePath || raw.ReviewerImagePath;
          if (img) setImagePreviewUri(formatImageUrl(img));
        }
      })
      .catch((err) => {
        console.error("Error fetching review by ID:", err);
        if (!passedItem) {
          setErrorMessage("Failed to load review details from server.");
        }
      })
      .finally(() => {
        setLoadingInitial(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let finalImagePath = formData.reviewerImagePath;
      let finalGivenName = formData.givenFileName;
      let finalOriginalName = formData.originalFileName;

      // 1. Upload new image if user selected a replacement
      if (selectedFile) {
        setUploadingImage(true);
        const uploadRes = await uploadTestimonialImage(
          selectedFile,
          formData.reviewerImagePath || ""
        );
      ...
      // 3. Call AddUpdate API
      const res = await addUpdateTestimonialReview(payload);

      if (res?.success || res?.isSuccess) {
        const msg = res?.message || "Testimonial review updated successfully!";
        ...
        throw new Error(res?.message || "Failed to update testimonial review.");
      }
    } catch (err) {
      console.error("Update testimonial review error:", err);
      setErrorMessage(err.message || "Failed to update testimonial review.");
    } finally {
      setSaving(false);
      setUploadingImage(false);
    }
  ...
  return (
    <div className="w-full pb-12">
      <PageMeta
        title="Edit Testimonial Review | IgnitoVerse Admin"
        description="Edit an existing customer testimonial review"
      />
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 pb-5 dark:border-gray-800">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">
            Edit Testimonial Review #{reviewId}
          </h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            Modify reviewer credentials, rating and commentary. Changes update the homepage immediately.
          </p>
        </div>
      ...
      {loadingInitial ? (
        <div className="flex h-72 items-center justify-center rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
          <div className="inline-flex items-center gap-3 text-sm text-gray-500">
            <div className="size-5 animate-spin rounded-full border-2 border-brand-500 border-t-transparent" />
            <span>Loading testimonial review data...</span>
          </div>
        </div>
      ) : (
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-xs dark:border-gray-800 dark:bg-gray-900">
              ...
              <div className="border-b border-gray-100 px-6 py-4.5 dark:border-gray-800">
                ...
                <p className="text-xs text-gray-400">
                  Update reviewer credentials, star rating, feedback commentary or reviewer profile picture.
                </p>
              </div>
                      ...
                      <p className="text-[11px] text-gray-400">
                        ...
                        {selectedFile ? (
                          <span className="ml-1 font-medium text-brand-600 dark:text-brand-400">
                            New: {selectedFile.name} ({(selectedFile.size / 1024).toFixed(0)} KB)
                          </span>
                        ) : formData.reviewerImagePath ? (
                          <span className="ml-1 font-mono text-[10px] text-gray-400 truncate">
                            {formData.originalFileName || formData.reviewerImagePath}
                          </span>
                        ) : null}
                      </p>
                    </div>
                  ...
                  >
                    {saving ? (
                      <>
                        ...
                        <span>{uploadingImage ? "Uploading Photo..." : "Updating..."}</span>
                      </>
                    ) : (
          ...
          <div className="xl:col-span-4">
            <div className="sticky top-20 rounded-2xl border border-gray-200 bg-white p-5 sm:p-6 shadow-xs dark:border-gray-800 dark:bg-gray-900">
              ...
              <div className="mb-4 flex items-center justify-between border-b border-gray-100 pb-3.5 dark:border-gray-800">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-2.5 py-0.5 text-[11px] font-semibold text-blue-600 dark:bg-blue-950/40 dark:text-blue-400">
